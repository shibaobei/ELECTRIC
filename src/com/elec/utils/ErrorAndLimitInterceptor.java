 ...
 * 细颗粒控制权限
 *    *使用struts2的拦截器
 *    *定义一个注解（mid，pid），对应权限code和父级的权限code，将注解添加到类中
 *      方法的上面
 *    *每一个Action类的方法上添加注解，表示方法的唯一标识，即该方法所具有的权限
 *    *在struts2的拦截器中，从session中获取角色id，获取action类表示方法上的注解，使用角色id，mid，pid
 *      查询角色权限表，判断当前用户是否可以操作该方法。
 */
